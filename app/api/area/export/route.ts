...
export async function GET(request: Request) {
  try {
    ...
    if (allAreas.length === 0) {
      return NextResponse.json(
        { success: false, message: "Tidak ada data Area untuk diekspor" },
        { status: 404 }
      );
    }

    // Format data untuk Excel
    const exportData = allAreas.map((area, index) => ({
      No: index + 1,
      "Kode Area": area.kode_area,
      "Nama Area": area.nama_area,
      "Keterangan": area.keterangan || "",
      "Tanggal Dibuat": area.createdAt ? new Date(area.createdAt).toLocaleDateString("id-ID") : "",
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Data Area");
    ...
    const filename = `Export_Area_${timestamp}.xlsx`;
    ...
    console.error("EXPORT AREA ERROR:", error);
    return NextResponse.json(
