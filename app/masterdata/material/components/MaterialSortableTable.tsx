...
import { MaterialBaaUsageDialog } from "./MaterialBaaUsageDialog";
...
export function MaterialSortableTable({
  ...
  const toggleSelectAll = () => {
    const allSelected = paginated.every((item) => selectedIds.has(item.id_material));
    if (selectAllPage && allSelected) {
      setSelectAllPage(false);
      setSelectedIds((prev) => {
        const next = new Set(prev);
        paginated.forEach((item) => next.delete(item.id_material));
        return next;
      });
    } else {
      ...
      setSelectedIds((prev) => {
        const next = new Set(prev);
        paginated.forEach((item) => next.add(item.id_material));
        return next;
      });
    }
  };
  ...
  const handleBulkExport = async () => {
    ...
    try {
      const url = ids.length === sorted.length
        ? `/api/material/export`
        : `/api/material/export?ids=${ids.join(",")}`;
      ...
      const filename = contentDisposition
        ...
        : `Export_Material_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      toast.success(`Berhasil mengekspor ${ids.length} data Material`);
    } catch (error) {
    ...
    <Card className="rounded-3xl border shadow-xl transition-all hover:shadow-2xl dark:bg-slate-900 dark:border-slate-800 dark:shadow-none">
                  ...
                  >
                    <div
                      className={`flex h-4 w-4 items-center justify-center rounded border-2 transition-colors ${
                        paginated.length > 0 && paginated.every((item) => selectedIds.has(item.id_material))
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <button
                    type="button"
                    onClick={() => toggleSelect(item.id_material)}
      ...
      {bulkDeleteOpen && bulkDeleteIds.length > 0 && (
        <MaterialDeleteDialog
          id={bulkDeleteIds[0]}
          namaMaterial={`${bulkDeleteIds.length} Material`}
