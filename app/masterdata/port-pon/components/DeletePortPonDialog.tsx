...
import { deletePortPon } from "../actions";

interface DeletePortPonDialogProps {
  id: number;
  name: string;
  bulkIds?: number[];
  open?: boolean;
...
export const DeletePortPonDialog = ({ id, name, bulkIds, open: controlledOpen, onOpenChange, onDeleteStart }: DeletePortPonDialogProps) => {
  ...
  const handleConfirm = () => {
    ...
    startTransition(async () => {
      try {
        if (isBulk && bulkIds) {
          // Bulk delete via API
          const response = await fetch(`/api/portpon/bulk-delete?ids=${bulkIds.join(",")}`, {
            method: "DELETE",
          });
          if (!response.ok) {
          ...
          toast.success(`Berhasil menghapus ${bulkIds.length} data Port PON`);
        } else {
          await deletePortPon(id);
          toast.success("Data Port PON berhasil dihapus");
        }
        setIsOpen(false);
        router.refresh();
      } catch (err: unknown) {
        const error = err as Error;
        setErrorMsg(error.message ?? "Gagal menghapus Port PON, coba lagi.");
        toast.error(error.message ?? "Gagal menghapus Port PON");
      }
    });
  };
      ...
      >
        <AlertDialogHeader className="!items-center !text-center sm:!text-center w-full flex-shrink-0 px-4 pt-4 sm:px-6 sm:pt-6">
          ...
          <AlertDialogTitle className="w-full text-center text-lg font-bold text-slate-900 dark:text-slate-50">
            {isBulk ? `Hapus ${bulkIds?.length} Port PON ini?` : "Hapus Port PON ini?"}
          </AlertDialogTitle>
          <AlertDialogDescription className="w-full text-center text-sm leading-relaxed text-slate-500 dark:text-slate-300">
            {isBulk ? (
              <>Kamu akan menghapus <strong className="text-slate-700 dark:text-slate-100">{bulkIds?.length} Port PON</strong> yang dipilih. Data yang sudah dihapus tidak bisa dikembalikan.</>
            ) : (
              <>Kamu akan menghapus Port PON <strong className="text-slate-700 dark:text-slate-100">&quot;{name}&quot;</strong>. Data yang sudah dihapus tidak bisa dikembalikan.</>
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
