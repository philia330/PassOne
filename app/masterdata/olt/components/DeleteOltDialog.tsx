...
export const DeleteOltDialog = ({ id, namaOlt, bulkIds, open: controlledOpen, onOpenChange, onDeleteStart }: DeleteOltDialogProps) => {
  ...
  const handleConfirm = () => {
    ...
    startTransition(async () => {
      try {
        if (isBulk && bulkIds) {
          // Bulk delete via API
          const response = await fetch(`/api/olt/bulk-delete?ids=${bulkIds.join(",")}`, {
            method: "DELETE",
          });
          if (!response.ok) {
          ...
          toast.success(`Berhasil menghapus ${bulkIds.length} data OLT`);
        } else {
          await deleteOlt(id);
          toast.success("Data OLT berhasil dihapus");
        }
        setIsOpen(false);
        router.refresh();
      } catch (err: unknown) {
      ...
      >
        <AlertDialogHeader className="!items-center !text-center sm:!text-center w-full flex-shrink-0 px-4 pt-4 sm:px-6 sm:pt-6">
          ...
          <AlertDialogTitle className="w-full text-center text-lg font-bold text-slate-900 dark:text-slate-50">
            {isBulk ? `Hapus ${bulkIds?.length} OLT ini?` : "Hapus data OLT ini?"}
          </AlertDialogTitle>
          <AlertDialogDescription className="w-full text-center text-sm leading-relaxed text-slate-500 dark:text-slate-300">
            {isBulk ? (
              <>Kamu akan menghapus <strong className="text-slate-700 dark:text-slate-100">{bulkIds?.length} OLT</strong> yang dipilih. Data yang sudah dihapus tidak bisa dikembalikan.</>
            ) : (
              <>Kamu akan menghapus OLT <strong className="text-slate-700 dark:text-slate-100">&quot;{namaOlt}&quot;</strong>. Data yang sudah dihapus tidak bisa dikembalikan.</>
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
