  ...
  const handleConfirm = () => {
    ...
    startTransition(async () => {
      try {
        if (isBulk && bulkIds) {
          // Bulk delete via API
          const response = await fetch(`/api/user/bulk-delete?ids=${bulkIds.join(",")}`, {
            method: "DELETE",
          });
          if (!response.ok) {
      ...
      >
        <AlertDialogHeader className="!items-center !text-center sm:!text-center w-full flex-shrink-0 px-4 pt-4 sm:px-6 sm:pt-6">
          ...
          <AlertDialogTitle className="w-full text-center text-lg font-bold text-slate-900 dark:text-slate-50">
            {isBulk ? `Hapus ${bulkIds?.length} user ini?` : "Hapus user ini?"}
          </AlertDialogTitle>
          <AlertDialogDescription className="w-full text-center text-sm leading-relaxed text-slate-500 dark:text-slate-300">
            {isBulk ? (
              <>Kamu akan menghapus <strong className="text-slate-700 dark:text-slate-100">{bulkIds?.length} User</strong> yang dipilih. Data yang sudah dihapus tidak bisa dikembalikan.</>
            ) : (
              <>Kamu akan menghapus user <strong className="text-slate-700 dark:text-slate-100">&quot;{name}&quot;</strong>. Data yang sudah dihapus tidak bisa dikembalikan.</>
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
